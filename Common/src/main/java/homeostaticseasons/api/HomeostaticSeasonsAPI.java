    ...
    public static @Nullable Season getCurrentSeason(Level level) {
        if (isSeasonalDimension(level.dimension())) {
            if (ConfigHandler.Common.seasonChangeMethod() == SeasonChangeMethod.REALTIME) {
                ...
                return ConfigHandler.Common.getSeasonFromDayTime(level.getDefaultClockTime());
            }
        }
        else {
            return null;
    ...
    public static long getTimeUntilNextSeason(Level level) {
        if (isSeasonalDimension(level.dimension())) {
            if (ConfigHandler.Common.seasonChangeMethod() == SeasonChangeMethod.REALTIME) {
                ...
                return ConfigHandler.Common.getTimeUntilNextSeason(level.getDefaultClockTime());
            }
        }
